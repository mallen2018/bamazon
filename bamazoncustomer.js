...
function inquire() {
	inquirer.prompt([{
			type: "input",
			name: "id",
			message: "What is the ID of the robot you would like to buy?",
		...
		},
		{
			type: "input",
			name: "quantity",
			message: "How many robots would you like?",
			validate: function (answer) {
				var input = answer;
				return new Promise((resolve, reject) => {
			...
			(err, results, fields) => {
				if (err) {
					...
					if (newStock > -1) {
						...
						console.log("Not enough robots left!");
						displayAll();
					}
				}
			})
